...
- 本机记录智能请求次数与服务返回的 token 数，不猜测模型价格，也不代你支付任何 AI 费用。
...
- BYOK 按需发送：点击“生成设置草案”后，应用只会发送该文本框里的描述，不发送已保存的工资、排班、窗口位置或历史记录。对应 AI 服务仍会依其自己的隐私政策处理这段文字。
...
点击组件底部的“用一句话调整” → “连接我的 AI”，填写你的兼容 OpenAI API 的地址、模型名称与 API Key。应用不预置或偏好任何服务商，也不会替你购买或调用付费模型。
